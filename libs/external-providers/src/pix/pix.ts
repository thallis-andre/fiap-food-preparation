// The code in this directory belongs to: https://github.com/guilhermeasn/react-qrcode-pix
...
export default class PIX {
  ...
  public static EVM(ID: number, content: string): string {
    ...
    if (content.length > 99)
      throw new Error(
        'O conteúdo do EVM pode ter no máximo noventa e nove caracters e espacos!',
      );
   ...
   */
  public static removeAccent(
    subject: string,
    extra_filter: RegExp | string = /[^\w\s]/gim,
  ): string {
    return !!String.prototype.normalize
      ? subject
          .normalize('NFD')
          .replace(/[\u0300-\u036f]/g, '')
          .replace(extra_filter, '')
      : subject.replace(extra_filter, '');
  }
  ...
  public constructor({
    ...
    if (!ignoreErrors) {
      ...
      if (!pixkey || !merchant || !city)
        throw new Error(
          'As propriedades: pixkey (chave pix), merchant (nome do recebedor) e city (cidade do recebedor), são obigatórios!',
        );

      // Verifica se a chave eh valida
      if (!PIX.verifyPixKey(pixkey))
        throw new Error(
          `A chave PIX (pixkey) '${pixkey}' parece ser inválida! Exemplos de formatos válidos: EMAIL: fulano_da_silva.recebedor@example.com | CPF: 12345678900 | CNPJ: 00038166000105 | TELEFONE: +5561912345678 | ALEATORIA: 123e4567-e12b-12d1-a456-426655440000`,
        );

      // Verifica se amount recebeu um valor valido
      if (typeof amount === 'number' && amount <= 0)
        throw new Error(
          'A propriedade amount deve receber um valor numérico maior que zero ou nulo!',
        );
    }
    ...
    this._props = {
      ...
      code:
        PIX.removeAccent(code || '', /[^a-z0-9]/gim)
          .substring(0, 25)
          .toUpperCase() || '***',
  ...
  public getMerchantCep(): string {
    return this._props.cep && /^\d{8}$/.test(this._props.cep)
